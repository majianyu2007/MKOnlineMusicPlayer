...
function welcome() {
	...
	console.log('Created by MaJianyu :) Source Code: https://github.com/DaltonWebDev/console.chat / My Blog: https://blog.majianyu.eu.org/');
}
function help() {
...
function loadMessages() {
	var myRequest = new Request('https://chat.majianyu.eu.org/api/read.php?domain=' + domain);
	fetch(myRequest)
...
function send(message, color = '000000') {
	...
	var myRequest = new Request('https://chat.majianyu.eu.org/api/send.php');
	if (usernameLS === null) {
...
var enabled = localStorage.getItem('enabled');
if (enabled === null) {
	console.log(`Want to chat with other people who are browsing on ${domain}? Awesome! Type cc() and hit enter to enable chat.majianyu.eu.org / Send cc() again at any time to stop loading in messages.`);
} else {
	loadMessages();
